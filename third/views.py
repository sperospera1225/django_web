...
def list(request):
    restaurants = Restaurant.objects.all().annotate(reviews_count=Count('review'))\
        .annotate(average_point=Avg('review__point')) # Aggregation
...
def update(request):
    if request.method == 'POST' and 'id' in request.POST:
        ...
        # item = Restaurant.objects.get(pk=request.GET.get('id')) ## third/update?id=2 가정
...
def detail(request, id):
    ...
    return HttpResponseRedirect('/third/detail/')
...
def review_list(request):
    ...
    reviews = Review.objects.all().select_related().order_by('-created_at') # join
